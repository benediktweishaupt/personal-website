...
<ProjectSection variant="full" gap="10">
  <ProjectDescription>How an opaque compliance wizard with a "9999 years" workaround became a modular, predictable system. Five clearly scoped modules, smart defaults, and a timeline visualisation replaced confusion with confidence.</ProjectDescription>
...
<ProjectSection variant="simple" gap="2">
  ...
  <ProjectTagList tags={["Lead Product Designer", "UX Systems", "Stakeholder Alignment"]} />
</ProjectSection>
...
<ProjectMediaGrid columns={1} items={[
  { type: "image", src: "/image/projects/reteach/compliance/form-exploration.png", alt: "Three iterations of compliance form fields", caption: "Design exploration: three iterations of the compliance form fields — from the existing multi-screen settings (top) through checkbox-based simplification (middle) to explicit 'never' selects replacing ambiguous empty states (bottom). Each iteration reduced cognitive load while preserving the same configuration power." },
]} />
...
<ProjectMediaGrid columns={1} items={[
  { type: "image", src: "/image/projects/reteach/compliance/modular-user-stories.png", alt: "Modular compliance requirements as user stories", caption: "Modular compliance requirements mapped as user stories. Each module — one-time deadlines, relative deadlines, recurring rules, recurring-relative rules — was scoped, designed, and tested independently." },
]} />
...
<ProjectMediaGrid columns={1} items={[
  { type: "image", src: "/image/projects/reteach/compliance/certificate-badges.png", alt: "Certificate status badges", caption: "Certificate status visualisation: colour-coded badges show validity at a glance — GÜLTIG (green), expiring soon (yellow), ABGELAUFEN (red), NICHT ERHALTEN (grey). This replaced the previous state where users had no visual feedback on compliance status." },
]} />

<ProjectMediaGrid columns={1} items={[
  { type: "video", src: "/video/projects/reteach/compliance/demo.mov", caption: "Compliance workflow demo — the complete module flow from configuration to certificate status" },
]} />
